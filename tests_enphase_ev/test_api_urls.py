...
async def test_api_builds_urls_correctly():
    ...
    assert any(
        ...
        for fragment in (
            "/service/evse_controller/9990001/ev_chargers/status",
            "/service/evse_controller/9990001/ev_charger/status",
        )
    )
    ...
    assert (
        "/service/evse_controller/9990001/ev_chargers/499900000001/start_charging"
        in start_call[1]
    )
    assert stop_call[0] == "PUT"
    assert (
        "/service/evse_controller/9990001/ev_chargers/499900000001/stop_charging"
        in stop_call[1]
    )
    assert trig_call[0] == "POST"
    assert (
        "/service/evse_controller/9990001/ev_charger/499900000001/trigger_message"
        in trig_call[1]
    )
