...
WARMUP_STATE_TASKS: tuple[RefreshTask, ...] = (
    method_task(
    ...
    method_task("storm_guard_s", "storm guard", "_async_refresh_storm_guard_profile"),
...
def warmup_energy_tasks(working_data: dict[str, dict]) -> tuple[RefreshTask, ...]:
    return (
        method_task(
...
def post_session_followup_tasks(day_local_default: object) -> tuple[RefreshTask, ...]:
    return (
        callback_task(
            "evse_timeseries_s",
            "EVSE timeseries",
