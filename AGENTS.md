...
- For codebase orientation beyond this file, use `docs/architecture.md` and `docs/glossary.md` before changing cross-cutting coordinator, API, diagnostics, or entity behavior.
...
- CI runs `pre-commit run --from-ref "origin/<base>" --to-ref HEAD` on pull requests unless `.pre-commit-config.yaml` or `.ruff.toml` changed, in which case it runs `pre-commit run --all-files`; mirror that scope locally when debugging CI-only hook failures.

## Maintenance Workflows
- Firmware catalog updates are generated by `.github/workflows/firmware-catalog.yml` with Python `3.14` and published to the `firmware-catalog` branch. To test generation locally, keep generated output outside the checkout, for example:
  - `docker compose -f devtools/docker/docker-compose.yml run --rm ha-dev bash -lc "python scripts/firmware_catalog.py --output-dir /tmp/firmware-catalog"`
- Service-status checks are generated by `.github/workflows/service-status.yml`, run every 30 minutes, and publish branch/wiki artifacts from `scripts/service_status.py`. Local runs need Enphase credentials in `ENPHASE_EMAIL` and `ENPHASE_PASSWORD`; optional site/device hints use `ENPHASE_SITE_ID`, `ENPHASE_SERIAL`, and `ENPHASE_LOCALE`.
  - `docker compose -f devtools/docker/docker-compose.yml run --rm ha-dev bash -lc "python scripts/service_status.py --output-dir /tmp/service-status"`
- Issue labels are reconciled by `.github/workflows/auto_label_issues.yml` via `scripts/issue_labeling.py`; run it only with the GitHub workflow environment or equivalent `GITHUB_TOKEN` and issue-form environment variables.
