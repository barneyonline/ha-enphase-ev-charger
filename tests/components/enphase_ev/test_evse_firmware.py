...
async def test_manager_uses_stale_cache_on_error() -> None:
    client = DummyClient(payload=[{"serialNumber": "482522020944"}])
    ...
    details = await manager.async_get_details()
    assert details == {"482522020944": {"serialNumber": "482522020944"}}
    status = manager.status_snapshot()
...
async def test_manager_returns_cached_details_after_waiting_for_lock() -> None:
    client = BlockingClient(payload=[{"serialNumber": "482522020944"}])
    manager = EvseFirmwareDetailsManager(lambda: client, ttl_seconds=300)
    ...
    assert await first == {"482522020944": {"serialNumber": "482522020944"}}
    assert await second == {"482522020944": {"serialNumber": "482522020944"}}
    assert client.calls == 1


def test_normalize_details_and_helpers_cover_edge_cases() -> None:
    payload = [
        {"serialNumber": "482522020944", "targetFwVersion": "1.0"},
        {"serialNumber": ""},
        "bad",
    ]
