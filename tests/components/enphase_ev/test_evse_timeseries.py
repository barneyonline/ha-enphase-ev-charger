...
async def test_evse_timeseries_manager_retains_cache_on_unavailable(hass) -> None:
    ...
    client.evse_timeseries_daily_energy = AsyncMock(
        side_effect=EVSETimeseriesUnavailable(
            "GET /service/timeseries/evse/timeseries/daily_energy?site_id=9990001&source=evse"
        )
    )
    client.evse_timeseries_lifetime_energy = AsyncMock(
        side_effect=EVSETimeseriesUnavailable(
            "GET /service/timeseries/evse/timeseries/lifetime_energy?site_id=9990001&source=evse"
        )
    )
    ...
    assert "9990001" not in str(diagnostics["last_error"])
    assert "9990001" not in str(diagnostics["daily"]["last_error"])
    assert "9990001" not in str(diagnostics["lifetime"]["last_error"])
